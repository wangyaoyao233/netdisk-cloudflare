...
## 2026-04-30 修复前端刷新后回到根目录
### 背景

前端当前目录和面包屑路径只保存在 React 内存状态中。用户进入某个子目录后刷新浏览器，页面会重新初始化为 `root`，导致回到根目录。

### 目标

让当前目录位置可以跨浏览器刷新恢复，同时保持现有无路由依赖的前端结构。

### 采用的修改

1. 新增 `utils/navigationState.ts`，负责从 URL 查询参数读取和写入当前文件夹位置。
2. 进入文件夹、点击面包屑返回时，同步更新 `folder` 和 `path` 查询参数。
3. `App.tsx` 初始化时从 URL 恢复 `currentParentId` 和 `pathStack`。
4. 监听浏览器 `popstate`，支持后退/前进时恢复目录层级。

### 结果

- `npm run build --workspace=frontend` 通过。
- `npm run lint --workspace=frontend` 通过。
- 在子目录刷新浏览器后，前端会留在刷新前的目录层级，并恢复面包屑显示。

### 本次的最佳实践总结

会影响页面位置的导航状态不应只放在组件内存里。即使暂时不引入路由库，也应把可恢复、可分享的导航状态同步到 URL，使刷新、后退和前进符合用户预期。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如果引入 React Router，可以把当前查询参数方案迁移成真正的目录路由。

## 2026-04-30 修复文件列表拖动误触发点击
### 背景

文件列表行支持单击预览或进入文件夹，但用户在列表中按住鼠标轻微拖动时，浏览器会产生文本选择或拖动体验，释放鼠标后仍可能触发行点击，导致误打开文件或目录。

### 目标

区分真实单击和拖动操作，避免拖动列表内容时触发预览、下载或进入目录等单击行为。

### 采用的修改

1. 在 `FileBrowser` 中记录行级 pointer down 起点。
2. pointer move 超过阈值后标记为拖动，后续 click 直接吞掉，不执行 `onPreview`。
3. 双击进入文件夹前同样检查拖动标记，避免拖动释放后的误触发。
4. 为文件行和返回上级行增加 `select-none`，减少拖动时出现文本选区的视觉干扰。
5. 将 `.. / Go back to parent` 返回上级行也纳入同一套 pointer 移动判断，避免该行拖动后误触发返回。

### 结果

- `npm run build --workspace=frontend` 通过。
- `npm run lint --workspace=frontend` 通过。
- 文件列表保留单击行为，但鼠标拖动后释放不会再被当作单击处理。
- 返回上级行也应用相同处理，拖动该行不会误触发返回目录。

### 本次的最佳实践总结

列表项上的点击动作应与指针移动距离解耦。只要界面没有实现完整的选择态、多选态和批量操作，拖动产生的框选或文本选区就不应触发业务动作，应明确抑制误点击。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如果要支持真正的多选，可以引入显式选择态、键盘辅助选择和批量操作栏，而不是依赖浏览器默认文本选择行为。

## 2026-04-30 统一文件夹返回交互
### 背景

文件列表中进入文件夹使用单击，面包屑导航也使用单击，但 `.. / Go back to parent` 返回上级行此前使用双击，和同类导航行为不一致。

### 目标

让进入文件夹、返回上一级和面包屑跳转保持一致的单击导航体验，同时保留拖动误触发保护。

### 采用的修改

1. 将返回上级行从 `onDoubleClick` 改为 `onClick`。
2. 返回上级行继续复用 pointer 移动距离判断，拖动释放后不会执行返回。

### 结果

- `npm run build --workspace=frontend` 通过。
- `npm run lint --workspace=frontend` 通过。
- 返回上一级现在可以单击触发，和进入文件夹、面包屑导航保持一致。

### 本次的最佳实践总结

同一类导航动作应保持一致的触发方式。只有在动作存在风险或需要防误触时才应提高交互门槛，否则双击会造成“点了没反应”的体验。
...
## 2026-05-01 替换前端原生弹窗提示
### 背景

前端在上传失败、删除确认、视频处理状态和新建文件夹等交互中使用了浏览器默认的 `alert`、`confirm` 和 `prompt`。这些原生弹窗样式不可控，和当前网盘系统的视觉风格不一致。

### 目标

用应用内统一的反馈组件替换原生弹窗，让确认、输入和错误提示保持一致的视觉风格，同时不改变现有文件管理业务流程。

### 采用的修改

1. 新增 `AppDialog` 组件，支持确认弹窗和输入弹窗两种模式。
2. 新增 `ToastViewport` 组件，用于展示成功、错误和信息提示。
3. 将删除确认、新建文件夹输入、上传失败、下载失败、预览失败、视频处理状态提示改为应用内反馈。
4. 保留原有文件上传、删除、下载、预览和目录刷新逻辑，只替换交互反馈层。

### 结果

- 前端代码中已无 `alert`、`confirm`、`prompt` 调用。
- 用户操作反馈与当前 Tailwind + Lucide 的界面风格保持一致。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

浏览器原生弹窗适合临时调试，但正式产品应把确认、输入和异步错误统一到应用内反馈系统中。这样可以保持视觉一致性，也便于后续扩展自动关闭、危险操作强调和移动端适配。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续可将 toast 和 dialog 状态抽成独立 hook，便于更多页面复用。

## 2026-05-01 调整前端 Modal 关闭与滚动行为
### 背景

图片预览、视频播放和确认/输入弹窗等 modal 场景会在点击空白遮罩区域时关闭，并且 modal 打开后页面背景仍可能响应滚动。

### 目标

让 modal 只能通过明确的关闭、取消或确认操作退出；同时在 modal 打开期间锁定背景页面滚动，避免误操作和视觉位置变化。

### 采用的修改

1. 移除图片预览 modal 遮罩层的点击关闭逻辑。
2. 移除视频播放 modal 遮罩层的点击关闭逻辑。
3. 移除应用内确认/输入弹窗遮罩层的点击关闭逻辑。
4. 在 `App` 中根据 `previewFile`、`videoFile`、`dialog` 状态统一判断 modal 是否打开，并在打开时设置 `document.body.style.overflow = 'hidden'`。
5. 背景滚动锁增加滚动条宽度补偿，减少锁定滚动时的页面横向跳动。

### 结果

- 点击 modal 外部空白区域不会再自动关闭。
- 打开任意 modal 时，背景页面不会继续滚动。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

涉及删除确认、输入表单和媒体播放的 modal，应避免使用遮罩点击作为默认关闭方式。明确的关闭入口更可控，也能减少误触；同时背景滚动锁应集中在 modal 状态层处理，避免不同弹窗组件各自实现出不一致的行为。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续可补充 `Escape` 键关闭策略，并按 modal 类型区分是否允许快捷关闭。

## 2026-05-01 禁用 Modal 打开期间的误选中文本
### 背景

Modal 打开后，用户在遮罩或内容区域长按、拖动鼠标时仍可能框选页面或弹窗内文字，视觉上会出现不必要的文本选中效果。

### 目标

在 modal 打开期间避免误选中文本，同时保留新建文件夹输入框的正常编辑能力。

### 采用的修改

1. 在 modal 打开时临时设置 `document.body.style.userSelect = 'none'`，关闭背景页面文字选择。
2. 在 modal 关闭时恢复原始 `userSelect` 样式，避免影响正常页面。
3. 为图片预览、视频播放和确认/输入弹窗根容器增加 `select-none`。
4. 为新建文件夹输入框单独增加 `select-text`，保留输入框内文本选择能力。

### 结果

- Modal 打开期间长按或拖动不会再出现不必要的文字框选效果。
- 输入类 modal 仍可正常输入和选中文本。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

交互遮罩不仅要处理关闭和滚动，也应处理文本选择这类细节状态。全局锁定适合背景页面，局部控件则应按需恢复能力，避免为了视觉细节牺牲表单可用性。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如增加可复制内容的 modal，可为具体文本区域单独恢复 `select-text`。

## 2026-05-01 增加前端目录列表缓存
### 背景

前端每次进入文件夹都会调用 `GET /api/items?parentId=...` 拉取目录列表。用户在已访问过的目录之间来回切换时，会产生重复请求和 loading 闪烁。

### 目标

为目录列表增加轻量级前端内存缓存，减少重复 API 请求，同时保留手动刷新和写操作后的数据一致性。

### 采用的修改

1. 在 `App` 中新增按 `parentId` 分组的目录列表内存缓存。
2. 进入已缓存目录时直接使用缓存内容，不再重复请求 API。
3. 点击刷新、上传成功、新建文件夹成功时强制重新拉取当前目录并更新缓存。
4. 删除成功后同步更新当前目录 UI 与缓存；删除文件夹时清理该文件夹自身缓存。
5. 为目录加载增加请求序号保护，避免快速切换目录时旧请求覆盖当前目录内容。

### 结果

- 已访问目录再次进入时会直接命中缓存。
- 手动刷新和写操作仍会重新请求后端，避免当前目录数据长期停留在旧状态。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

目录列表这类读多写少的数据适合先使用按资源 ID 分组的前端内存缓存。缓存命中可以优化导航体验，但所有改变目录内容的写操作都必须同步更新或失效缓存；同时异步请求需要竞态保护，避免慢请求回写过期状态。

### TODO(如果需要的话，一些将来可以做的事情)

- 如果后续增加分页、搜索或跨端同步需求，可以评估引入 TanStack Query 统一处理缓存、失效和后台刷新策略。

## 2026-05-01 提取目录列表缓存 Hook
### 背景

目录列表缓存最初直接写在 `App.tsx` 中，虽然改动较小，但缓存命中、强制刷新、缓存更新和请求竞态保护都属于独立的数据加载职责。继续放在页面入口组件中会增加 `App` 的维护负担。

### 目标

将目录列表缓存和加载逻辑提取为专用 hook，让 `App` 保留页面编排、上传、删除、新建文件夹、预览和导航等业务流程。

### 采用的修改

1. 新增 `useFolderFiles` hook，集中管理目录列表状态、loading 状态、按 `parentId` 分组的内存缓存和请求序号保护。
2. hook 暴露 `loadFiles`、`updateFolderCache` 和 `clearFolderCache`，供页面在刷新、删除和清理文件夹缓存时调用。
3. `App` 改为通过 hook 获取 `files` 和 `loading`，不再直接持有缓存 `Map` 与请求序号。
4. 保持原有缓存行为不变：进入已缓存目录直接命中缓存，手动刷新和写操作强制重新拉取，删除操作同步更新当前目录缓存。

### 结果

- 目录缓存职责从页面组件中独立出来，`App` 的状态职责更清晰。
- 缓存行为和用户交互没有变化。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

当一个页面组件中的逻辑开始形成独立生命周期和状态边界时，应优先提取为小 hook，而不是继续扩展页面组件。hook 的边界应围绕可复用的数据职责建立，避免把导航、弹窗和具体业务动作一起搬进去造成过度抽象。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如增加更多页面级异步状态，可继续按职责提取 hook，并评估是否需要统一的数据请求层。

## 2026-05-01 增加文件和文件夹重命名
### 背景

前端文件列表已有上传、下载、预览、删除和创建文件夹能力，但缺少对文件或文件夹显示名称的修改入口。当前存储对象路径与显示名称分离，适合通过更新 D1 元数据实现轻量重命名。

### 目标

支持在前端对文件和文件夹执行重命名，并保证同级目录不出现同名项目，重命名后列表、下载文件名和面包屑状态保持一致。

### 采用的修改

1. Worker 新增 `PATCH /api/items/:id` 接口，只更新 `items.name` 和 `updatedAt`，不移动或重写 R2 对象。
2. 后端对新名称进行 trim 校验，并检查同级目录下是否已有同名项目，冲突时返回 `409`。
3. 前端 `FileService` 新增 `renameItem` 方法，并保留 HTTP status 供 UI 区分重名冲突。
4. 文件列表 Actions 中新增重命名按钮，文件和文件夹均可使用。
5. 输入弹窗支持 `initialValue`，重命名时默认填入当前名称。
6. 重命名成功后同步更新当前目录缓存，并在命中面包屑路径时同步更新 URL 中的路径状态。

### 结果

- 文件和文件夹都可以从列表中打开重命名弹窗并提交新名称。
- 同级重名会被 Worker 拒绝，前端显示明确错误提示。
- 重命名只影响元数据，不影响 R2 原始文件、HLS 产物或缩略图路径。
- `npm run build --workspace=frontend` 已通过。
- `npm run test --workspace=worker` 已通过。
- `npx tsc -p worker/tsconfig.json --noEmit` 已通过。

### 本次的最佳实践总结

当对象存储 key 与业务显示名已经解耦时，重命名应优先落在元数据层，避免不必要的对象复制和删除。涉及目录列表排序和缓存的写操作，应在成功响应后同步更新本地缓存，并把唯一性约束放在服务端保证数据一致性。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如增加移动文件或批量重命名，可考虑抽取统一的项目元数据更新接口和更细粒度的错误码。

## 2026-05-01 将重命名和删除收纳到更多菜单
### 背景

文件列表在增加重命名后，Actions 区域同时暴露预览、刷新、下载、重命名、删除和更多按钮，视觉上偏拥挤，并且删除作为高风险操作直接显示在行内更容易误触。

### 目标

保留文件预览、刷新、下载等高频快捷操作，将重命名和删除放入竖向更多菜单中，让管理类操作更集中、更安全。

### 采用的修改

1. 在文件列表组件中增加当前展开菜单的本地状态。
2. 竖向更多按钮点击后展开行内菜单，菜单包含 `Rename` 和 `Delete`。
3. 菜单项点击时先关闭菜单，再调用现有重命名或删除流程。
4. 点击页面其他位置或按 `Escape` 时关闭已展开菜单。
5. 删除菜单项使用红色危险样式，重命名使用普通操作样式。

### 结果

- 重命名和删除不再作为独立行内按钮直接暴露。
- 点击竖向更多按钮后可以选择重命名或删除。
- 菜单交互不会触发行点击预览或进入文件夹。
- `npm run lint --workspace=frontend` 已通过。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

文件管理类界面应区分高频快捷操作和低频管理操作。删除这类高风险动作适合放入更多菜单或确认流程中，减少误触；菜单展开状态应具备点击外部和键盘退出能力，并与行点击事件隔离。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如果更多菜单继续增加操作，可考虑抽取通用的行操作菜单组件。

## 2026-05-01 将下载操作收纳到更多菜单
### 背景

文件列表的 Actions 区域已将重命名和删除收纳进更多菜单，但下载仍作为文件行内快捷按钮展示。随着操作项增多，行内按钮仍然偏密集。

### 目标

进一步简化文件行的快捷操作，将下载与重命名、删除一起放入竖向更多菜单，同时保留视频处理中刷新这类状态相关快捷动作。

### 采用的修改

1. 移除文件行内独立的下载按钮。
2. 在更多菜单中为文件类型项目增加 `Download` 菜单项。
3. 文件夹菜单不显示下载项，保持操作语义清晰。
4. 菜单中的下载点击后关闭菜单，再调用现有下载流程。

### 结果

- 文件下载、重命名和删除集中在更多菜单中。
- 文件夹菜单仍只包含重命名和删除。
- 视频 pending/processing 状态仍保留行内刷新快捷按钮。
- `npm run lint --workspace=frontend` 已通过。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

表格行内快捷按钮应只保留和当前状态强相关、需要高频触达的操作。下载、重命名、删除这类针对项目的管理动作可以集中进更多菜单，减少视觉噪音，并让文件和文件夹的交互模型更一致。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续可根据菜单项数量和复用需求，将更多菜单抽成独立组件。

## 2026-05-01 常驻显示文件操作入口
### 背景

文件列表 Actions 区域之前依赖 hover 展示操作按钮。桌面端可用，但移动端没有稳定 hover 行为，用户不容易发现更多菜单，也可能在尝试唤出操作时误触发行点击。

### 目标

让文件和文件夹的操作入口在所有设备上都直接可见，提升移动端可用性，同时避免 Actions 区域重新变得拥挤。

### 采用的修改

1. 移除 Actions 区域的 hover 透明度控制，操作入口常驻显示。
2. 移除文件行内预览快捷按钮，继续通过点击行触发预览或进入文件夹。
3. 保留视频 `pending` / `processing` 状态的刷新快捷按钮。
4. 竖向更多菜单保持常驻显示，承载下载、重命名和删除等管理操作。

### 结果

- 移动端和桌面端都能直接看到更多操作入口。
- 普通文件和文件夹的 Actions 区域更简洁。
- 视频处理中状态仍可快速刷新。
- `npm run lint --workspace=frontend` 已通过。
- `npm run build --workspace=frontend` 已通过。
...
触屏场景不应依赖 hover 作为关键操作入口。列表行可以保留点击作为主操作，但更多菜单这类管理入口应常驻可见；同时应控制常驻按钮数量，避免为了可发现性牺牲移动端布局。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续可补充移动端实际 viewport 截图检查，确认 Actions 区域在窄屏表格横向滚动下仍然易用。

## 2026-05-01 为文件列表底部菜单预留空间
### 背景

文件列表的更多菜单使用行内绝对定位展开。当最后一行打开更多菜单时，菜单会贴近表格底部，视觉上容易让列表高度临时变化或显得没有足够承载空间。

### 目标

在文件列表底部预留一段稳定余白，让最后一项的更多菜单展开时有足够空间，避免列表底部因菜单展开产生突兀变化。

### 采用的修改

1. 在 `tbody` 有文件且非 loading 状态下追加一个 `aria-hidden` 的 spacer 行。
2. spacer 行跨 4 列并提供固定高度，只作为视觉和菜单展开空间，不参与实际数据展示。
3. 空目录状态不添加 spacer，避免影响空状态布局。
...
表格中的浮层菜单如果靠近容器边缘，应提前为展开状态预留空间，避免交互时才改变布局。用于布局缓冲的 DOM 应明确标记为 `aria-hidden`，避免被误认为真实数据行。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如果更多菜单改为 portal 浮层，可移除表格底部 spacer，并改由浮层定位系统处理边界翻转。

## 2026-05-01 命名化文件列表菜单底部预留高度
### 背景

文件列表底部 spacer 最初使用固定高度，能覆盖当前更多菜单，但后续如果菜单项增加，固定值容易被忽略，导致最后一行菜单再次贴近底部。

### 目标

让底部预留高度的维护意图更明确，并为后续菜单项增加预留更充足空间。

### 采用的修改

1. 新增 `ACTION_MENU_BOTTOM_SPACE_CLASS` 常量集中定义底部 spacer 高度。
2. 将 spacer 从 `h-24` 调整为更保守的 `h-40`。
3. 添加简短注释，说明在菜单改为浮层前需要让该值与最高菜单保持同步。

### 结果

- 底部预留空间与更多菜单高度的关系更容易被后续维护者发现。
- 后续增加少量菜单项时有更充足的缓冲空间。
- `npm run lint --workspace=frontend` 已通过。
- `npm run build --workspace=frontend` 已通过。
...
当布局依赖固定预留值时，应把魔法值命名化并说明维护约束。这样虽然不如浮层定位系统彻底，但能在当前复杂度下减少后续改菜单时遗漏相关布局约束的风险。

### TODO(如果需要的话，一些将来可以做的事情)

- 如果更多菜单继续扩展或需要适配更复杂的容器边界，应改为 portal/floating 菜单并移除 spacer。

## 2026-05-01 修正更多菜单打开时的行点击行为
### 背景

文件列表更多菜单展开后，点击其他文件或文件夹会在关闭菜单的同时触发行主操作，导致进入文件夹、返回上级或触发文件预览/下载兜底。这与“点击外部先关闭菜单”的常见交互预期不一致。

### 目标

当更多菜单已打开时，点击菜单外的列表行只关闭菜单，不执行该行的主操作，避免误进入目录或误触发文件操作。

### 采用的修改

1. 在文件列表组件中增加 `closeActionMenu` 统一关闭函数。
2. 修改文件行单击逻辑：菜单打开时先关闭菜单并直接返回。
3. 修改文件行双击逻辑：菜单打开时关闭菜单并阻止进入文件夹。
4. 修改父级 `..` 行点击逻辑：菜单打开时只关闭菜单，不返回上级目录。
5. 保留菜单内部点击阻止冒泡，菜单项自身仍可执行下载、重命名和删除。

### 结果

- 菜单打开后点击其他文件行不会触发预览或下载。
- 菜单打开后点击其他文件夹行不会进入文件夹。
- 菜单打开后点击 `..` 行不会返回上级。
- 菜单项本身的操作不受影响。
- `npm run lint --workspace=frontend` 已通过。
- `npm run build --workspace=frontend` 已通过。
...
浮层菜单打开时，外部点击的第一职责应是关闭当前浮层，而不是继续执行底层元素的主操作。对于列表行这类高影响点击区域，应在行级 handler 中显式识别菜单展开状态，避免依赖 document 级点击监听造成冒泡顺序问题。

### TODO(如果需要的话，一些将来可以做的事情)

- 如果未来抽取通用行操作菜单，可将“菜单打开时外部点击吞掉主操作”的规则封装到组件或 hook 中。

## 2026-05-01 优化文件列表标题与目录状态文案
### 背景

文件列表顶部原有的 `Files & Folders`、`Manage your cloud storage` 和面包屑中的 `My Files` 信息较泛，无法直观表达当前位置和当前目录内容状态。

### 目标

让文件管理界面顶部文案更直白：根目录入口、当前目录标题和目录内容统计都应直接服务于导航和浏览判断。

### 采用的修改

1. 将面包屑根目录入口从 `My Files` 调整为 `All files`。
2. 文件列表标题改为动态展示：根目录显示 `All files`，子目录显示当前文件夹名。
3. 文件列表副标题改为目录状态：加载中显示 `Loading contents...`，空目录显示 `This folder is empty`，有内容时显示文件夹和文件数量。
4. 在 `App` 中将当前文件夹名传入 `FileBrowser`，避免文件列表组件自行推断导航路径。

### 结果

- 顶部文案能直接表达当前位置和当前目录内容。
- 原有搜索、新建文件夹、上传等操作入口保持不变。
- `npm run lint --workspace=frontend` 已通过。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

文件管理类界面的标题区应优先承载导航和状态信息，而不是使用泛化说明文案。组件需要展示当前目录时，应由上层导航状态显式传入必要信息，避免重复维护路径推断逻辑。

### TODO(如果需要的话，一些将来可以做的事情)

- 后续如果加入搜索功能，可在搜索态将标题切换为 `Search results` 并显示匹配数量。

## 2026-05-01 将页脚说明改为个人署名
### 背景

页脚原先展示 `Built with React & Cloudflare Stack (Worker, D1, R2)`，这类技术栈说明对普通文件管理界面使用者帮助有限。

### 目标

将页脚改为更直接的个人署名展示，减少无关技术信息。

### 采用的修改

1. 将页脚说明文案替换为 `CloudNet by 王雨蒙`。
2. 保留现有页脚品牌图标、排版和低干扰视觉样式。
...
面向使用者的界面文案应优先表达产品归属、状态或操作价值。技术栈信息更适合放在 README 或开发文档中，而不是占用应用界面的长期视觉位置。

### TODO(如果需要的话，一些将来可以做的事情)

- 如需更强个人品牌感，可后续统一调整 header 与 footer 的品牌命名策略。

## 2026-05-01 固定文件列表列宽并省略长文件名
### 背景

文件列表使用浏览器默认的自动表格布局，列宽会随内容动态计算。文件名过长时容易挤压大小、创建时间和操作列，影响列表扫描和对齐稳定性。

### 目标

让文件列表列宽保持稳定，并在文件名超出 Name 列可用空间时用省略号展示，同时保留查看完整文件名的能力。

### 采用的修改

1. 将文件列表表格改为 `table-fixed`，并设置最小宽度，窄屏下继续通过横向滚动保持布局可用。
2. 通过 `colgroup` 固定 Name、Size、Created At 和 Actions 四列宽度。
3. 为文件名所在 flex 容器补充 `min-w-0`，确保 `truncate` 能在固定列内生效。
4. 为文件名文本增加 `title`，悬停时可以查看完整名称。

### 结果

- 长文件名会在 Name 列内以省略号截断。
- Size、Created At 和 Actions 列不再被长文件名挤压。
- `npm run build --workspace=frontend` 已通过。

### 本次的最佳实践总结

表格型文件管理界面应优先保证列对齐稳定性。对于可变长文本，固定外层列宽并在文本容器上明确 `min-width: 0`，比只给文本元素添加省略样式更可靠。

### TODO(如果需要的话，一些将来可以做的事情)

- 如需进一步优化手机端体验，可将窄屏文件列表改为专门的紧凑布局，而不是仅依赖横向滚动。

## 2026-05-01 修复顶部提示框长文本溢出
### 背景

文件删除、重命名、创建等操作后的顶部提示框会展示文件名。文件名很长或包含连续字符时，提示框文本可能超出弹层边界。

### 目标

让顶部 toast 提示框在展示长文件名、长错误信息时仍保持在弹层宽度内，避免文字溢出。

### 采用的修改

1. 为 toast 标题和消息文本增加 `overflow-wrap: anywhere`，允许连续长字符串在必要时断行。
2. 为 toast 图标和关闭按钮增加 `shrink-0`，避免文本换行时压缩操作控件。
3. 保留现有提示框宽度、位置和视觉样式。
...
任何可能展示用户输入或文件名的紧凑 UI，都应显式处理连续长字符串。只依赖普通空格换行不足以覆盖文件名、URL、错误码等真实输入场景。

### TODO(如果需要的话，一些将来可以做的事情)

- 如果后续 toast 内容更复杂，可考虑为文件名类内容提供中间省略展示，同时在详情或 tooltip 中保留完整文本。
